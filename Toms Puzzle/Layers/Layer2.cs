...
namespace Toms_Puzzle.Layers
{
    /*
        ...
        For example, here is the byte 0xB2 (178 in decimal):

          1 0 1 1 0 0 1 0 <-- Parity bit (least significant bit)
          ^ ^ ^ ^ ^ ^ ^
          | | | | | | |
            Data bits

        Of the data bits above, four of them are 1s. This is an even
        number, so the 0 parity bit is correct.
